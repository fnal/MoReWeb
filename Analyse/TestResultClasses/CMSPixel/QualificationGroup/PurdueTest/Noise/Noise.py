...
class TestResult(AbstractClasses.GeneralTestResult.GeneralTestResult):
    def CustomInit(self):
        self.Name='CMSPixel_QualificationGroup_PurdueTest_Noise_TestResult'
    ...
    def SpecialPopulateData(self,TestResultObject, Parameters):
        ...
        TestResultObject.ResultData['Plot']['ROOTObject'] =  ROOT.TH1D(TestResultObject.GetUniqueID(), '', TestResultObject.ParentObject.Attributes['NumberOfChips'], TestResultObject.ParentObject.Attributes['StartChip'], TestResultObject.ParentObject.Attributes['StartChip'] + TestResultObject.ParentObject.Attributes['NumberOfChips'])
        
        # stores the integral values
        TestResultObject.ResultData['Plot']['ROOTObject_h2'] =  ROOT.TH1D(TestResultObject.GetUniqueID(), '', TestResultObject.ParentObject.Attributes['NumberOfChips'], TestResultObject.ParentObject.Attributes['StartChip'], TestResultObject.ParentObject.Attributes['StartChip'] + TestResultObject.ParentObject.Attributes['NumberOfChips'])
        ...
        if TestResultObject.ResultData['Plot']['ROOTObject']:
            ...
            for i in TestResultObject.ParentObject.ResultData['SubTestResults']['Chips'].ResultData['SubTestResults']:
                ...
                if Parameters.has_key('CalcFunction'):
                    Value = Parameters['CalcFunction'](Value, ChipTestResultObject.ResultData['SubTestResults'][Parameters['DataKey']].ResultData['KeyValueDictPairs'])
            ...
            lineB = Line.DrawLine(TestResultObject.ParentObject.Attributes['StartChip'], YLimitB, TestResultObject.ParentObject.Attributes['StartChip'] + TestResultObject.ParentObject.Attributes['NumberOfChips'], YLimitB)
