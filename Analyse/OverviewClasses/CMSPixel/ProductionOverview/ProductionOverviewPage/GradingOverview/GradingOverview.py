...
class ProductionOverview(AbstractClasses.GeneralProductionOverview.GeneralProductionOverview):
    ...
    def GenerateOverview(self):
        ...
        ### Full Qualification
        TableData.append(
            [{'Class' : 'Value', 'Value' : 'T = -20'}, {'Class' : 'Value', 'Value' : "%d"%len([x for x in FTMinus20BTC_Grades if x=='A']) }, {'Class' : 'Value', 'Value' : "%d"%len([x for x in FTMinus20BTC_Grades if x=='B']) }, {'Class' : 'Value', 'Value' : "%d"%len([x for x in FTMinus20BTC_Grades if x=='C']) }]
        )
        #TableData.append(
